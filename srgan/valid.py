...
def srgan_valid(path):
    ...
    gen = srgan_generator(t_image, is_train=False)
    ...
    tl.visualize.save_image(out[0], samples_valid_dir_srgan + 'gen.png')
    tl.visualize.save_image(valid_lr_img, samples_valid_dir_srgan + 'lr.png')
    tl.visualize.save_image(valid_hr_img, samples_valid_dir_srgan + 'hr.png')

    bicub = scipy.misc.imresize(valid_lr_img, [size[0]*4, size[1]*4], interp='bicubic', mode=None)
    tl.visualize.save_image(bicub, samples_valid_dir_srgan + 'bicubic.png')
